...
switch ($type)
{
	...
	case 'redactor':
		$css = array_merge($css, array('class' => 'redactor'));
		...
		echo $form->textArea($model, $field, array_merge($css, array('rows' => 5)));
		break;
	...
	case 'file':
		...
		?>
			<span class="file-input-name" data-ng-controller="ModalDemoCtrl">
				...
				<?php if ($is_image): ?>
					<a href="<?= $destination ?>" data-trigger="openimage" data-src="<?= $destination ?>"><?= $model->$field ?></a>
				<?php else: ?>
	...
	case 'dropdown_multiple':
		echo $form->dropDownList($model, $field, $data, array_merge($css, array('multiple' => 'multiple')));
		break;
	case 'password':
		$model->$field = '';
